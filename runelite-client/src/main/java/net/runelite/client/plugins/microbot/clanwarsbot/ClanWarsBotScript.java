...
public class ClanWarsBotScript {
    ...
    public boolean interactPortal() {
        // Check if player is below y 4700
        if (Rs2Player.getWorldLocation().getY() > 4700) {
            return false;
        }
    ...
    private void handleCombat() {
        ...
        for (Player player : players) {
            ...
            boolean isTarget = targetNames.stream()
                    .map(String::toLowerCase)  // Convert each target name to lowercase
                    .anyMatch(name -> playerNameLower.equals(name));  // Compare lowercase to lowercase
            ...
            // CUSTOM: Updated rules for CUSTOM_LOCATION
            if (selected == ClanWarsLocation.CUSTOM_LOCATION) {
                // Skip targets below Y 4759
                if (targetLoc.getY() <= 4759) {
                    continue;
                }

                // If target is above Y 4800, we can attack regardless of combat state
                if (targetLoc.getY() >= 4800) {
                    // Continue with the attack logic (no restrictions)
                }
                // For targets in Y range 4760-4799
                else if (targetLoc.getY() >= 4760 && targetLoc.getY() <= 4799) {
                    // If we're in that range too (4760-4799)
                    if (myLoc.getY() >= 4760 && myLoc.getY() <= 4799) {
                        // Neither can be in combat
                        if (localPlayer != null && localPlayer.getInteracting() != null) {
                            continue;
                        }
                        if (player.getInteracting() != null) {
                    ...
                    // If we're above Y 4800, target cannot be in combat
                    else if (myLoc.getY() >= 4800) {
                        if (player.getInteracting() != null) {
                            continue;
                        }
                    }
                    // If we're below Y 4760, skip this target
                    else {
                        continue;
                    }
                }
